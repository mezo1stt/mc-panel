...
//   players, macAddress, lastUpdate
// }
const servers = new Map();

// أوامر معلقة لكل سيرفر
const pendingCommands = new Map(); // uuid -> { type, createdAt }
...
app.post("/plugin/register", (req, res) => {
  ...
  const record = {
    ...
    macAddress: String(body.macAddress ?? existing?.macAddress ?? "Unknown"),
...
app.post("/api/servers/:uuid/shutdown", (req, res) => {
  ...
  return res.json({ ok: true, queued: true, type: "shutdown", serverUuid: uuid });
});

// أمر ban
app.post("/api/servers/:uuid/ban", (req, res) => {
  if (!requireToken(req, res)) return;
  ...
  return res.json({ ok: true, queued: true, type: "ban", serverUuid: uuid });
});
