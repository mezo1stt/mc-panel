...
const storage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    const safeOriginal = String(file.originalname || "update.bin").replace(/[^\w.\-]/g, "_");
...
app.get("/plugin/poll", (req, res) => {
  ...
  if (cmd) pendingCommands.delete(serverUuid);
...
app.get("/api/servers", requirePanelAuth, (_req, res) => {
  ...
  return res.json({ ok: true, servers: list });
});
...
app.post("/api/servers/:uuid/update-upload", requirePanelAuth, upload.single("updateFile"), (req, res) => {
  const uuid = String(req.params.uuid || "").trim();
  if (!servers.has(uuid)) {
    if (req.file?.path) {
